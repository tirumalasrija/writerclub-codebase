							...
							case 'post_type':
								...
								if ( $rule->applies_to( 'content_type', 'taxonomy' ) ) {
									...
									if ( ! empty( $rule_object_ids ) && is_array( $rule_object_ids ) ) {
										...
										if ( ! in_array( get_post_type( $object_id ), (array) $taxonomy_object->object_type, true ) ) {
											break;
										}
										...
										// if there are no terms specified, does it have any terms from that particular taxonomy?
										$taxonomy_name   = $rule->get_content_type_name();
										$taxonomy_object = get_taxonomy( $taxonomy_name );

										// sanity check: is the taxonomy currently registered for the post type?
										// get_the_terms does not care about this, so we need to make sure we do!
										if ( $taxonomy_object && in_array( get_post_type( $args['object_id'] ), (array) $taxonomy_object->object_type, true ) ) {
